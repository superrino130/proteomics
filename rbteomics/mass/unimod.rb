...
module Unimod
  ...
  def _formula_parser(formula, session)
    ...
    formula.split.each do |token|
      m = /(?<isotope>\d+)?(?<elemet>[^\(]+)(?:\((?<count>-?\d+)\))?/.match(token)
      if m.nil?.!
  ...
  #       isotope, element = /(?<isotope>\d+)?(?<element>\S+)/.match(symbol).captures
  #       if isotope
  ...
  #       m = /(?<isotope>\d+)?(?<element>\S+)/.match(symbol)
  ...
  #   session = sessionmaker(bind: engine, autoflush: false).new
  ...
  #   get = lambda do |identifier, strict: true|
