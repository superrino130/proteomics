...
class TestQvalues < Minitest::Test
  def setup
    @key = lambda { |x| x[0] }
  ...
  def test_qvalues_pep
    ...
    _run_check_pep(q)
    q = Target_decoy.qvalues(@psms, pep: @pep, key: @key)
    _run_check_pep(q)
  end

  def test_qvalues_with_decoy
    q = Target_decoy.qvalues(@psms, key: @key, is_decoy: @is_decoy, remove_decoy: false)
    _run_check(q, 2)
    q = Target_decoy.qvalues(@psms, key: @key, is_decoy: @is_decoy, remove_decoy: false, 'formula' => 1)
    _run_check(q, 1)
  end

  def test_qvalues_full_output
    q = Target_decoy.qvalues(@psms, key: @key, is_decoy: @is_decoy, remove_decoy: false, full_output: true)
    _run_check(q, 2)
  end
  ...
  def test_qvalues_pep_full_output
    ...
    _run_check_pep(q)
    q = Target_decoy.qvalues(@psms, 'key' => @key, 'pep' => @pep, 'full_output' => true)
    _run_check_pep(q)
  end

  def test_qvalues_from_numpy
    dtype = [['score', Numpy.int8], ['label', Numpy.str_, 1], ['pep', Numpy.float64]]
    psms = Numpy.array(@psms.to_a, dtype: dtype)
    q = Target_decoy.qvalues(psms, 'key' => @key, 'is_decoy' => @is_decoy, 'remove_decoy' => false, 'formula' => 1)
    _run_check(q, 1)
    q = Target_decoy.qvalues(psms, 'key' => @key, 'is_decoy' => @is_decoy, 'remove_decoy' => false, 'formula' => 1, 'full_output' => true)
    _run_check(q, 1)
  ...
  def test_qvalues_from_dataframe
    dtype = [['score', Numpy.int8], ['label', Numpy.str_, 1], ['pep', Numpy.float64]]
    psms = Pandas.DataFrame(Numpy.array(@psms.to_a, dtype: dtype))
    q = Target_decoy.qvalues(psms, 'key' => @key, 'is_decoy' => @is_decoy, 'remove_decoy' => false, 'formula' => 1)
    _run_check(q, 1)
    q = Target_decoy.qvalues(psms, 'key' => @key, 'is_decoy' => @is_decoy, 'remove_decoy' => false, 'formula' => 1, 'full_output' => true)
    _run_check(q, 1)
  ...
  def test_qvalues_empty_dataframe
    dtype = [['score', Numpy.int8], ['label', Numpy.str_, 1], ['pep', Numpy.float64]]
    psms = Pandas.DataFrame(Numpy.array([], dtype: dtype))
    q = Target_decoy.qvalues(psms, 'key' => @key, 'is_decoy' => @is_decoy, 'remove_decoy' => false, 'formula' => 1)
